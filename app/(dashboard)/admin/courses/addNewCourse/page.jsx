...
import { addCourse } from "@/app/stateManager/features/courses/courseSlice";
import {
  Plus,
  Trash2,
  Video,
...
const formInputStyle =
  "block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-gray-400 focus:border-transparent";
...
const AddNewCourse = () => {
  ...
  const removeChapter = (index) => {
    ...
    setActiveChapterIndex(
      Math.min(activeChapterIndex, updatedChapters.length - 1)
    );
  };
  ...
  const removeLesson = (chapterIndex, lessonIndex) => {
    ...
    setActiveLessonIndex(
      Math.min(
        activeLessonIndex,
        updatedChapters[chapterIndex].lessons.length - 1
      )
    );
  };
    ...
    switch (lesson.contentType) {
      ...
      case "video":
        return (
          <div>
            <label
              className={`${formLabelStyle} flex items-center justify-center px-4 py-6 border-2 border-dashed rounded-md cursor-pointer hover:border-blue-500`}
            >
              <input
                type="file"
                accept="video/*"
              ...
              {lesson.content ? (
                <div className="text-center">
                  ...
                  <p className="text-sm font-medium">
                    {lesson.contentFile?.name || "Video selected"}
                  </p>
            ...
            >
              ...
              {lesson.content ? (
                <div className="text-center">
                  ...
                  <p className="text-sm font-medium">
                    {lesson.contentFile?.name || lesson.content}
                  </p>
                </div>
              ) : (
                <div className="text-center">
  ...
  const handleSubmit = async (e) => {
    ...
    formData.chapters.forEach((chapter, cIndex) => {
      chapter.lessons.forEach((lesson, lIndex) => {
        if (lesson.contentFile) {
          uploadData.append(
            `chapter-${cIndex}-lesson-${lIndex}-file`,
            lesson.contentFile
          );
        }
      });
    ...
    const courseData = {
      ...formData,
      thumbnail: formData.thumbnailPreview, // This would be replaced with server URL
      chapters: formData.chapters.map((chapter) => ({
        ...chapter,
        lessons: chapter.lessons.map((lesson) => ({
          ...lesson,
          content: lesson.contentFile
            ? lesson.contentFile.name
            : lesson.content,
    ...
    <div className="max-w-3xl mx-auto bg-white rounded-lg shadow p-6">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="space-y-4 mb-6">
          ...
          {/* Thumbnail Upload */}
          <div>
            <label className={formLabelStyle}>Course Thumbnail</label>
            <label
              className={`flex items-center justify-center px-4 py-6 border-2 border-dashed rounded-md cursor-pointer hover:border-blue-500 ${
            ...
            <div className="space-y-6">
              ...
              {formData.chapters.map((chapter, cIndex) => (
                <div
                  key={cIndex}
                  className={`space-y-4 ${
                    activeChapterIndex === cIndex ? "block" : "hidden"
                  }`}
                >
                  ...
                  {/* Chapter Title */}
                  <div>
                    <label className={formLabelStyle}>Chapter Title</label>
                    <input
                      type="text"
                      value={chapter.title}
                      onChange={(e) =>
                      ...
                      <div className="space-y-4">
                        ...
                        {chapter.lessons.map((lesson, lIndex) => (
                          ...
                          >
                            ...
                            {/* Content Input */}
                            <div>
                              <label className={formLabelStyle}>Content</label>
