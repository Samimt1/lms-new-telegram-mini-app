        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {features.map((feature, index) => (
            <motion.div
              ...
              whileTap={{ scale: 0.95 }}
              className="p-6 bg-white dark:bg-[#0B1120] rounded-lg shadow-lg text-center transition-transform duration-300"
            >
              <div className="text-4xl sm:text-5xl">{feature.icon}</div>
              <h3 className="text-lg sm:text-xl md:text-2xl font-semibold mt-4">
                {feature.title}
              </h3>
              <p className="text-gray-600 dark:text-gray-300 mt-2 text-sm sm:text-base md:text-lg">
                {feature.desc}
              </p>
            </motion.div>
          ))}
